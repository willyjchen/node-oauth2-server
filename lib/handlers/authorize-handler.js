...
function AuthorizeHandler(options) {
  ...
  this.allowEmptyNonce = options.allowEmptyNonce;
  ...
  return Promise.all(fns)
    ...
    .spread(function(expiresAt, client, user) {
      ...
      return Promise.bind(this)
        ...
        .then(function(code) {
          var responseType = new ResponseType(code.authorizationCode, client, user, scope, this.options);
          var redirectUri = this.buildSuccessRedirectUri(uri, responseType);

          this.updateResponse(response, redirectUri, state, nonce);
...
AuthorizeHandler.prototype.getNonce = function(request) {
  ...
  /* Default allow empty nonce */
  if (this.allowEmptyNonce !== undefined && !this.allowEmptyNonce && !nonce) {
    throw new InvalidRequestError('Missing parameter: `nonce`');
  }

  if (nonce && !is.vschar(nonce)) {
    throw new InvalidRequestError('Invalid parameter: `nonce`');
  }
...
AuthorizeHandler.prototype.getUser = function(request, response) {
  let nonce = this.getNonce(request);
  ...
  if (this.authenticateHandler instanceof AuthenticateHandler) {
    return this.authenticateHandler.handle(request, response).get('user')
      .then(function (user) {
        /* not copy null nonce */
        return Object.assign(user, nonce ? nonce : undefined);
      });
  }
...
AuthorizeHandler.prototype.updateResponse = function(response, redirectUri, state, nonce) {
